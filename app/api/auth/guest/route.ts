...
import { guestSessionSchema } from '@/server/modules/auth/auth.validator';

export const dynamic = 'force-dynamic';

export const POST = createRouteHandler(
  { body: guestSessionSchema, rateLimit: AUTH_ATTEMPTS },
  async ({ request, body }) => {
    await database.connect();

    return authController.guest(request, body);
  },
);
